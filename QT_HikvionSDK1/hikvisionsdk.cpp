...
//        qDebug() << (m_stDevList.pDeviceInfo);
//        if (NULL == pDeviceInfo)
//        {
//            continue;
//        }
...
//                //qDebug()<<   ((char*)pDeviceInfo->SpecialInfo.stUsb3VInfo.idProduct);
//                qDebug() << (int)pDeviceInfo->SpecialInfo.stUsb3VInfo.idProduct;//unsigned short       ;   idProduct;;
...
int  HikvisionSDK::connectCamera(int id)
{
    ...
    for (unsigned int i = 0; i < m_stDevList.nDeviceNum; i++)
    {
        ...
        if (NULL == pDeviceInfo)
        {
            ...
            if(pDeviceInfo->nTLayerType==MV_USB_DEVICE)
            {
                if(id== (int)pDeviceInfo->SpecialInfo.stUsb3VInfo.idProduct)
                    {
                    m_Device= m_stDevList.pDeviceInfo[i];
                    break;
                    }
...
//读取相机中的图像
int HikvisionSDK::ReadBuffer(HObject &image)
{
